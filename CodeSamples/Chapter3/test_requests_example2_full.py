...
class TestHeroCode(unittest.TestCase):
    ...
    def test_get_hero_names(self):
        result = list(requests_example2.get_hero_names(self.fake_heroes))
        self.assertEqual(result, self.fake_heroes["members"])
    ...
    def test_get_hero_names_age_filter_no_results(self):
        result = list(
            requests_example2.get_hero_names(
                self.fake_heroes, hero_filter=lambda x: x.get("age", 0) > 50
            )
        )
